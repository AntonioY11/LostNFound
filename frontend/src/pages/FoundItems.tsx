...
export default function FoundItems(){
  ...
  useEffect(()=>{ fetch((import.meta.env.VITE_API_BASE||'http://localhost:3001') + '/api/found').then(r=>r.json()).then(setItems) },[])
  ...
  async function submit(e:React.FormEvent){
    ...
    const res = await fetch((import.meta.env.VITE_API_BASE||'http://localhost:3001') + '/api/found', { method:'POST', body: fd, headers: token ? { 'Authorization': 'Bearer ' + token } : undefined })
